...
public class CollectorAccessPolicy(AccessContext context) : AccessPolicyBase(context)
{
    ...
    public override IQueryable<Event> FilterEvents(IQueryable<Event> query) => 
        query.Where(e => !e.IsPrivate || Context.EventIds.Contains(e.Id) || (e.BranchId != Guid.Empty && Context.BranchIds.Contains(e.BranchId)));

    public override IQueryable<RecipientFund> FilterFunds(IQueryable<RecipientFund> query) => 
        query.Where(f => Context.FundIds.Contains(f.Id) || Context.EventIds.Contains(f.EventId));

    public override IQueryable<Contribution> FilterCollections(IQueryable<Contribution> query) => 
        query.Where(c => c.Receipt != null && c.Receipt.RecordedByUserId == Context.UserId);
