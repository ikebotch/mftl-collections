...
public record GetPublicReceiptQuery(Guid ReceiptId) : IRequest<PublicReceiptDto?>;

public class GetPublicReceiptQueryHandler(IApplicationDbContext dbContext) : IRequestHandler<GetPublicReceiptQuery, PublicReceiptDto?>
{
    public async Task<PublicReceiptDto?> Handle(GetPublicReceiptQuery request, CancellationToken cancellationToken)
    {
